...
const SheetSimulator: React.FC<Props> = ({ tasks, onTaskSubmit, currentTab }) => {
  ...
  // 1. FORMAT NGÀY: 05 - Jan - 2026
  const formatDateDisplay = (dateStr: string) => {
    if (!dateStr || dateStr === 'N/A' || dateStr.trim() === '') return '-';
    
    const date = new Date(dateStr);
    if (isNaN(date.getTime())) return dateStr; // Nếu không phải ngày chuẩn thì trả về text gốc
  ...
  // 2. XỬ LÝ LINK: Ẩn link dài -> Hiện chữ "Link" + Icon
    ...
    if (url.startsWith('http')) {
        return (
            <a 
              ...
              target="_blank" 
              rel="noreferrer" 
              className="inline-flex items-center gap-2 px-3 py-1 bg-[#00f3ff]/10 border border-[#00f3ff]/40 rounded text-[#00f3ff] hover:bg-[#00f3ff] hover:text-black transition-all text-[9px] font-bold uppercase tracking-wider"
              title={url} // Hover vào sẽ thấy link gốc
            >
          ...
          <tr className="bg-[#0d0b0a] sticky top-0 border-b border-[#00f3ff]/20 z-10 shadow-lg">
            {currentTab === '05' && (
              <>
                ...
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-32 text-center">PLAN END</th>
              </>
            )}
            ...
            {currentTab === '06' && (
              <>
                ...
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-32 text-center">NGÀY (C)</th>
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-32 text-center">STATUS (D)</th>
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-40">ANGLE (F)</th>
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-32 text-center">SEEDING (H)</th>
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-32 text-center">CONTENT (I)</th>
                <th className="p-4 heritage-font font-bold text-[#d4af37] border-r border-[#ffffff]/5 w-24 text-center">KÍCH HOẠT</th>
              </>
            )}
          </tr>
        </thead>
            ...
            <tr key={r.id} className="border-b border-[#ffffff]/5 hover:bg-[#00f3ff]/5 transition-colors group">
              ...
              {currentTab === '05' && (
                <>
                  ...
                  <td className="p-4 border-r border-[#ffffff]/5 text-center code-font text-[#d4af37]">
              ...
              {currentTab === '06' && (
                <>
                  ...
                  {/* Cột Seeding (H) - Dạng Link */}
                  <td className="p-4 border-r border-[#ffffff]/5 text-center">
                      {renderLink(r.seeding)}
                  </td>
                  
                  {/* Cột Content (I) - Dạng Link */}
                  <td className="p-4 border-r border-[#ffffff]/5 text-center">
                      {renderLink(r.contentBody)}
                  </td>
...
// 3. SỬA LỖI STATUS: Thêm whitespace-nowrap và min-width để không bị vỡ
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`code-font px-3 py-1 rounded-sm text-[9px] font-bold uppercase border tracking-widest whitespace-nowrap inline-flex items-center justify-center min-w-[70px] ${style}`}>
       {status || 'UNKNOWN'}
    </span>
  );
};
