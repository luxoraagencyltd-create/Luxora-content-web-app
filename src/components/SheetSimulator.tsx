...
const SheetSimulator: React.FC<Props> = ({ tasks, onTaskSubmit, currentTab }) => {
  ...
  // Hàm render link (icon hoặc text)
  const renderLink = (url?: string, label: string = "Link") => {
    if (!url || url === '#' || url.trim() === '') return <span className="text-[#888]/30">-</span>;
    
    // Nếu là link http -> hiện icon bấm được
    if (url.startsWith('http')) {
        return (
            <a href={url} target="_blank" rel="noreferrer" className="text-[#00f3ff] hover:text-white hover:underline code-font text-[9px] flex items-center justify-center gap-1 bg-[#00f3ff]/10 px-2 py-1 rounded border border-[#00f3ff]/30 transition-all">
                <i className="fa-solid fa-link"></i> {label}
            </a>
        );
    }
    // Nếu là text thường -> hiện text
  ...
  return (
    <div className="flex-1 overflow-auto bg-[#1a1412] rounded-xl shadow-inner">
      <table className="w-full text-left text-[11px] border-collapse min-w-[1400px]"> {/* Tăng min-width để đủ chỗ */}
        <thead>
          <tr className="bg-[#0d0b0a] sticky top-0 border-b border-[#00f3ff]/20 z-10 shadow-lg">
            
            {/* --- HEADER TAB 05 (Giữ nguyên) --- */}
            {currentTab === '05' && (
              <>
            ...
            <tr key={r.id} className="border-b border-[#ffffff]/5 hover:bg-[#00f3ff]/5 transition-colors group">
              ...
              {currentTab === '05' && (
                <>
                  ...
                  <td className="p-4 border-r border-[#ffffff]/5 text-center code-font text-[#d4af37]">{r.planEnd}</td>
                </>
              )}
              ...
              {currentTab === '06' && (
                <>
                  ...
                  <td className="p-4 text-[#888] border-r border-[#ffffff]/5 code-font tracking-wider text-center text-[10px] whitespace-nowrap">{r.planEnd}</td>
                  
                  <td className="p-4 border-r border-[#ffffff]/5 text-center align-middle">
                      <StatusBadge status={r.status} />
                  </td>

                  <td className="p-4 font-bold text-[#e0e0e0] border-r border-[#ffffff]/5 heritage-font tracking-wider text-[10px] italic">{r.name}</td>
                  
                  {/* Seeding & Content */}
                  <td className="p-4 border-r border-[#ffffff]/5 text-[9px] text-[#00f3ff]">{renderClickableContent(r.seeding)}</td>
                  <td className="p-4 border-r border-[#ffffff]/5 text-[9px] text-[#e0e0e0]">{renderClickableContent(r.contentBody)}</td>
              ...
              <td className="p-4 text-center bg-[#00f3ff]/5">
                {r.status === 'Review' ? (
                  <div className="flex flex-col gap-2">
                    <button onClick={() => onTaskSubmit('approve', r.id)} className="heritage-font bg-[#00f3ff] text-black px-3 py-1.5 rounded-sm shadow-[0_0_10px_rgba(0,243,255,0.3)] text-[9px] font-black hover:bg-white transition-all w-full clip-path-slant">
                      <i className="fa-solid fa-check mr-1"></i> DUYỆT
                    </button>
                    <button onClick={() => onTaskSubmit('request_edit', r.id)} className="heritage-font bg-transparent border border-[#ff003c] text-[#ff003c] px-3 py-1.5 rounded-sm text-[9px] font-black hover:bg-[#ff003c] hover:text-white transition-all w-full clip-path-slant">
...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`code-font px-3 py-1 rounded-sm text-[8px] font-bold uppercase border tracking-widest whitespace-nowrap inline-flex items-center justify-center min-w-[60px] ${style}`}>
       {status || 'Unknown'}
    </span>
  );
};
